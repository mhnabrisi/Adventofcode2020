...
"8-9 v: vvvvvvvvvg",
            ...
            foreach (var item in entries)
            {
                ...
                var split1 = items[0].Split("-");
                int min = Convert.ToInt32(split1[0]);
                int max = Convert.ToInt32(split1[1]);

                char letter = items[1].Trim(':').FirstOrDefault();
                var password = items[2];

                if (password.Count() >= max - 1)
                {
                    var charCount = password.Count(x => x == letter);
            ...
            Console.WriteLine($"Part2 Result : {part1Count}");
            Console.WriteLine($"Part2 Result : {part2Count}");
